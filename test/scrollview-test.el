  ...
  (scrollview-test--with-displayed-buffer
    ...
    (let ((window (selected-window))
      ...
      (cl-letf (((symbol-function 'window-default-font-height)
                 (lambda (_window) 20))
                ((symbol-function 'scrollview--refresh-now)
                 (lambda (&rest _)
                   (setq called t))))
...
(ert-deftest scrollview-scroll-throttling-keeps-one-timer-per-window ()
  (scrollview-test--reset-state)
  (let ((window (selected-window))
        (scrollview-update-interval 1)
        called)
    (dotimes (_ 20)
      (scrollview--after-window-scroll window nil))
    (let ((timer (gethash window scrollview--scroll-refresh-timers)))
      (should (timerp timer))
      (should (= (hash-table-count scrollview--scroll-refresh-timers) 1))
      (cancel-timer timer))
    (cl-letf (((symbol-function 'scrollview--refresh-now)
               (lambda (&optional target scroll)
                 (setq called (list target scroll)))))
  ...
  (scrollview-test--with-displayed-buffer
    ...
    (let ((window (selected-window))
      ...
      (cl-letf (((symbol-function 'window-end)
                 (lambda (&rest _)
                   (ert-fail "scroll state requested exact window-end")))
                ((symbol-function 'window-vscroll)
                 (lambda (_window &optional _pixels) vscroll)))
        (let ((state (scrollview--render-state window)))
...
(ert-deftest scrollview-spell-collector-uses-flyspell-overlays ()
  ...
  (with-temp-buffer
    (insert "good\nbadword\nagain\n")
    (goto-char (point-min))
    (let ((scrollview-spell-checker 'flyspell)
          (overlay (make-overlay (line-beginning-position 2)
                                 (line-end-position 2))))
      (overlay-put overlay 'flyspell-overlay t)
...
(ert-deftest scrollview-spell-collector-selects-configured-checker ()
  (scrollview-test--reset-state)
  (with-temp-buffer
    (insert "flyspell\njinx\n")
    (goto-char (point-min))
    (let ((flyspell-overlay (make-overlay (line-beginning-position 1)
                                          (line-end-position 1)))
          (jinx-overlay (make-overlay (line-beginning-position 2)
                                     (line-end-position 2))))
      (overlay-put flyspell-overlay 'flyspell-overlay t)
      (overlay-put jinx-overlay 'category 'jinx-overlay)
      (let ((scrollview-spell-checker 'flyspell))
        (should (equal (scrollview--collect-spell-lines nil) '(1))))
      (let ((scrollview-spell-checker 'jinx))
        (should (equal (scrollview--collect-spell-lines nil) '(2)))))))
